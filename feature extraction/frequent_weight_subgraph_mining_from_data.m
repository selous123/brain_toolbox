function [model] = frequent_weight_subgraph_mining_from_data(net_mat,label)
type=4; % select type this is useless indeed
level=1; % select the level or the deepth of the search tree.
fold_num = 1; % this is useless indeed
parameter_num = 12;%the model parameter number which should be saved to do next step.
%for function net mining
if(type==4)
...
name = 'ad';
% set the pos mining parameter
% thres means the frequent threshold when we mining,the deeper the thres we
% can accept i
...
pos_down_bound=0.03; %when judge the ordered weight ,we should accept it when it is bigger enough so it is right in computer.
pos_up_bound = 0.35;
    
% set the neg mining parameter
...
neg_down_bound=0.03; %when judge the ordered weight ,we should accept it when it is bigger enough so it is right in computer.
neg_up_bound = 0.35;
end
disp('start mining.');
%% 挖掘
for fi=1:fold_num
    ...
    et=clock;
    disp(['the time of mining ',num2str(fi),'-th fold negsubgs is ',num2str(etime(et,st))]);
    ...
    et=clock;
    disp(['the time of mining ',num2str(fi),'-th fold possubgs is ',num2str(etime(et,st))]);
end
...
%% search the frequent subgraph ,the weight order was saved and others are dropped.
[node_num,~,net_num]=size(net_mat);
...
% load('edge2edge_weight.mat','edge2edge_weight');
%% edge2edge_weight(i,j,k)=1 denotes the k-th sample's edge[（fix(i/node_num),mode(i,node_num)]
% is bigger alpha than edge[（fix(j/node_num),mode(j,node_num)）]
% save the information of edge pairs,if weight of edge i is bigger thsn j ,then [i,j]will be true ,else fal
edge2edge_weight(edge_num,edge_num,net_num)=false;
edge_idx1=1;
% calculate edge2edge_weight
tic
for ni=1:node_num-1
...
toc
% save('edge2edge_weight.mat','edge2edge_weight');
%% change the edge index from (i,j)to k.
% edge_idx_mat(i,j)=(i-1)*node_num+(j-i),i<j
% edge_idx_mat(i,j)=(j-1)*node_num+(i-j),i>j
% edge_idx_mat(i,i)=0,i=1,2,...,node_num
...
%subgraph mining,we recursively add edge to our subgraph,this edge should be connected with our subgraph,and
%the edge's weight is smallest in our subgraph,then we calculate the
%frequence of our new subgraph,if it's frequence is big enough,we use this
%new subgraph to invoke the function itself,if it is not frequent enough we
%go to the next step.

global  subgraph_cell;
global  score_cell;
contain_edge_num=size(subgraph,1);
% if we have complete the level mining ,exit.
if(contain_edge_num-1>=level)
    return;
end
contain_node=unique(subgraph);
contain_node_num=length(contain_node);
% calculate our subgraph's score in every network.
score=logical(ones(1,net_num));
...
% for every point,we search the edge connected to this point and append the edge's annother point to subgraph.
for ni=1:contain_node_num
   for nj=1:node_num
%        disp('-----------------------------');
%        disp(subgraph);
%        disp([contain_node(ni),nj,contain_edge_num]);
%        pause;
       % judge if it is a loop
      if(nj==contain_node(ni))
          continue;
      end
      % judge if the edge has been contained.
      is_contain=false;
      ...
      % append the new edge to our subgraph
